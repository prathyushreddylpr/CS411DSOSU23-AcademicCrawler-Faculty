def find_possible_list(path_dict):
    # only use names and positions to calculate common_structure (further tests needed)
    d = {'Name': path_dict['Name'] if 'Name' in path_dict else [], 'Position': path_dict['Position'] if 'Position' in path_dict else []}
    # d = path_dict

    # for i in d:
    #     print(i, d[i])
    candidate_num = sum([len(d[i]) for i in d.keys()])
    common_structure = []
    a = 0
    while True:
        count = {}
